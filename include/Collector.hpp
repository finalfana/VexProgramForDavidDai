...
namespace Collector
{
    ...
    private:
        ...
        static const int ColorDetectWindow = 20; // 收集多少个采样数据
        static const int ColotDetectInterval = 10; //采样间隔时间 

        // 颜色阈值
        struct ColorThresholds 
        {
            double sat_min   = 20.0; // 饱和度低于这个就当背景
            double bri_min   = 5.0;  // 太暗当作无效
            double bri_max   = 95.0; // 太亮（反光/过曝）也当无效
        };
        ...
        std::vector<DetectedColor> ColorRecords;
        ...
        static void DetectFunction(void* Parameter)
        {
            ...
            {
                This -> ColorDetect();
                pros::delay(ColotDetectInterval);
            }
        }
    ...
    public:
        CollectorControl(pros::MotorGroup &_LiftMotors, pros::Motor &_SelectMotor, pros::Optical &_SelectColorSensor, SelectColor _MyColor, SelectColor _EnemyColor)
            : LiftMotors(_LiftMotors), SelectMotor(_SelectMotor), SelectColorSensor(_SelectColorSensor), MyColor(_MyColor), EnemyColor(_EnemyColor) 
        {
        ...
        void LiftUp()
        {
            LiftMotors.move_velocity(200);//待定数据
            SelectMotor.move_velocity(200);//待定数据
        }
        ...
        DetectedColor GetColorRecordAverage()
        {
            double hue_sum = 0.0;
            double sat_sum = 0.0;
            double bri_sum = 0.0;
            ...
            for  (DetectedColor &record : ColorRecords)
            {
                sat_sum += record.sat;
                bri_sum += record.bri;
        ...
        void ColorDetect()
        {
            ...
            else 
            {
                ...
                DetectedColor AvgColor = GetColorRecordAverage();
                // 判断是否检测到物体
                bool satisfied = (AvgColor.sat >= ColorThresholds().sat_min) && (AvgColor.bri >= ColorThresholds().bri_min) && (AvgColor.bri <= ColorThresholds().bri_max);
